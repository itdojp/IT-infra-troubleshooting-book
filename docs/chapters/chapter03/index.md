...
複雑なシステム問題に対処する効果的なアプローチの一つは、分割統治法（Divide and Conquer）の適用です。
手に負えない複雑な問題を「解決可能な小さな問題」に分解し、調査範囲を段階的に狭めます。

**システム境界の明確化**では、まず問題の全体的な境界を定義し、その中でより小さな境界を設定します。

- **物理的境界**: サーバー、ネットワークセグメント、データセンターなどの物理境界に基づいて問題領域を分割します。物理的に分離されたコンポーネント間では相互作用が限定され、問題の伝播パターンを絞りやすくなります。
- **論理的境界**: アプリケーション、サービス、データベース、プロセスなどの論理境界に基づいて問題領域を分割します。機能的な独立性と依存関係を反映し、影響範囲の把握に有効です。
- **時間的境界**: 問題発生前後の時間軸を分析し、変化が発生した時点を特定します。原因になりうる事象の範囲を限定できます。

**階層的分解の戦略**では、システムの階層構造に従って問題を分解します。

- **上位階層から下位階層への分解**: ユーザー体験レベルから、アプリケーション、ミドルウェア、OS、ハードウェアへと段階的に分解します。各階層の正常性を確認し、問題が存在する階層を特定します。
- **機能分解**: 認証、データ処理、通信など、機能単位で問題を分解します。機能的に独立した部分での問題発生状況を分析します。
- **依存関係による分解**: コンポーネント間の依存関係に基づいて問題を分解します。依存関係グラフを活用し、波及経路と影響範囲を把握します。
...
- **時系列比較**: 同一システムの過去状態と現在状態を比較します。同じ時間帯、同じ曜日、同じ月の過去データと比較することで、周期的変動を考慮できます。
- **負荷正規化比較**: 処理量、ユーザー数、データサイズなどの負荷要因を考慮し、負荷レベルを正規化した上で比較します。単純な数値比較では見落としやすい本質的な変化を検出できます。
- **環境比較**: 同一構成の異なる環境（本番、ステージング、開発環境）で比較し、環境特有の要因を特定します。

**差分分析の深化**では、単純な数値比較を超えて、差分の性質と意味を分析します。

- **パターン差分**: データのパターンや分布の変化を分析します（例: 平均値は変わらないが分散が増大、周期性が失われる）。
- **相関関係の変化**: 通常は相関があるメトリクス間で、相関が失われていないかを確認します。相関が崩れている場合、その間に問題が存在する可能性があります。
- **異常値の分析**: 正常範囲から外れる値の頻度、程度、パターンを分析します。散発的な異常値と継続的な異常値では、原因の性質が異なります。
...
物理的接続の確認により、ケーブルの接続状態、リンクライトの状態、ポートの物理的状態を確認します。`ethtool` コマンドを使用し、リンク状態、速度、デュプレックス設定を確認します。なお、`mii-tool` は旧来の `net-tools` に含まれるコマンドであり、環境によってはインストールされていません。
...
IPアドレス設定の確認により、IPアドレス、サブネットマスク、デフォルトゲートウェイの設定が正しいかを確認します。`ip addr show` で現在の設定を確認し、期待値と比較します。なお、`ifconfig` は旧来の `net-tools` に含まれるコマンドであり、環境によってはインストールされていません。

ルーティングテーブルの分析により、パケットが適切な経路で転送されるかを確認します。`ip route show` でルーティングテーブルを確認し、宛先への経路が存在するかを確認します。なお、`route` は旧来のコマンドであり、基本は `ip route` を前提にすると整理しやすくなります。

到達性テストにより、`ping`コマンドを使用して基本的な到達性を確認します。段階的に近い機器から遠い機器へと到達性をテストし、通信が途切れる地点を特定します。

**トランスポート層の診断**では、TCP/UDPの接続とポートの問題を確認します。

ポート接続性の確認により、`nc`（netcat）コマンドを使用して特定のポートへの接続を確認します。ファイアウォールやポートフィルタリングの問題を特定できます。なお、`telnet` は旧来の確認手段であり、環境によっては利用できない（インストールされていない）場合があります。

ソケット状態の分析により、`ss` コマンドを使用してソケットの状態を確認します。LISTEN状態、ESTABLISHED状態、TIME_WAIT状態などのソケット状態から、アプリケーションの動作状況を推測できます。なお、`netstat` は旧来の `net-tools` に含まれるコマンドであり、基本は `ss` を前提に整理すると混乱が少なくなります。

### アプリケーションスタックの階層的診断

Web アプリケーションシステムでは、複数の技術レイヤーが連携してサービスを提供します。各レイヤーでの診断により、問題の所在を効率的に特定できます。
...
pingコマンドの高度な活用
- `ping -i 0.1 target`: 高頻度テスト（0.1秒間隔。高負荷になり得るため短時間/対象限定で実施）
- `ping -M do -s 1472 target`: Path MTU の確認（Linux。DFを立ててフラグメントを禁止）
- `ping -f target`: フラッド ping テスト（高負荷。原則は検証環境でのみ。事前合意と停止手順が必要）
...
注: パケットには認証情報や個人情報などの機密情報が含まれる可能性があります。保存先/共有範囲/保管期間は、組織のセキュリティポリシーに従って管理してください。
...
- 分割統治（境界の設定・階層的分解）により、複雑な障害を「切り分け可能な問題」に分解する。
- 正常状態（ベースライン）との差分を定量的に比較し、変化点を特定して原因候補の範囲を狭める。
- 仮説（演繹・帰納・アブダクション）を構築し、影響度・確率・コストで優先順位付けしたうえで検証する。
- レイヤーモデル（OSI/TCP/IP 等）と適切なツール選択により、再現性のある診断手順として運用できる。

## 次章への接続

第3章で習得した論理的な切り分けと原因特定の手法を、第4章では各技術レイヤーでの具体的なトラブルシューティングに応用します。ネットワーク、OS、アプリケーション、データベースの各レイヤーでの実践的な診断技術を学びます。

次に読む： [第4章：各レイヤーでのトラブルシューティング実践](../chapter04/) / [目次（トップ）](../../)
