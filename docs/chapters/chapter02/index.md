...
初動で迷いやすいのは「何から見ればよいか」が定まらず、一次情報が揃わない状態です。ここでは、事象が曖昧（例:「遅い」「繋がらない」）な段階でも、最小の確認で状況を固定するための例を示します。

| 観点 | 取得例（Read-only / 読み取り専用） | 判断基準（例） | 次アクション（例） |
| --- | --- | --- | --- |
| 事象の再現 | 影響ユーザー/機能で再現確認（ブラウザ/`curl` など） | 再現条件が特定できる/できない | 再現条件をメモし、以後の検証を同条件に固定 |
| 影響範囲 | 対象ユーザー/リージョン/ネットワーク/機能の切り分け | 影響が局所/全体か | 影響が局所なら境界（ネットワーク/サーバー群）を狭める |
| 直近の変化 | デプロイ/設定変更/IaC変更/メンテの有無 | 変化点と事象開始時刻が近いか | 近い場合は「切り戻し（停止線含む）」を候補に入れる |
| リソース | `uptime`/`free -h`/`df -h`/（可能なら）メトリクス | CPU/メモリ/ディスク枯渇があるか | 枯渇がある場合は原因（負荷増/リーク/容量）へ分岐 |
| サービス状態 | `systemctl status <service>`、待受（`ss -lntp`） | 起動失敗/再起動ループ/待受なし | 設定/依存先/権限/リソースの観点で絞り込む |
| ログ（直近） | `journalctl -u <service> --since ...` / アプリログ | 同一エラーの反復、依存先エラー、OOM など | 代表ログを抜粋し、仮説（依存先/設定/負荷）を更新 |

ログ確認（例）:

```bash
# [Read-only] systemd 管理サービスの直近ログ（例: nginx）
sudo journalctl -u nginx --since "30 min ago" --no-pager | tail -n 50
```

ログ例（抜粋）:

```text
... nginx[1234]: connect() failed (111: Connection refused) while connecting to upstream, client: 203.0.113.10, upstream: "http://10.0.0.12:8080/"
```

判断基準（例）:

- `Connection refused` は「接続先ポートが待ち受けていない」可能性が高い（接続先プロセス停止、FW、宛先誤りなど）。まず接続先のサービス状態/待受を確認します。
- `Killed process ... (Out of memory)` があれば、メモリ枯渇（OOM）を疑い、直前の変化点（デプロイ/設定/負荷）と合わせて見ます。
...
| 時刻 | 事象/観測 | 判断/アクション | 根拠（ログ/メトリクス等） |
|---|---|---|---|
| 09:00 | アラート発報（エラー率上昇） | オンコールが対応開始を宣言 | ダッシュボード |
| 09:05 | 影響範囲を確認 | 重大度を暫定判定し関係者に共有 | 影響ユーザー数/主要機能の再現 |
| 09:10 | 切り分け開始 | 仮説Aを立て、優先度順に検証 | 直前の変更/ログ |
...
次に読む： [第3章：論理的な切り分けと原因特定](../chapter03/) / [目次（トップ）](../../)
