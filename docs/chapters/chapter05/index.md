...
**想定学習時間**: 5〜6時間（本文読解 + 章末の確認/演習を含む目安）
...
## 章末テンプレ（症状定義→影響範囲→仮説→Cheap test→恒久対策）

現場での記録/引き継ぎ用に、次の順序でまとめると再利用しやすいです。

1. **症状定義**: 何が起きているか（いつから/どの条件で/再現性）
2. **影響範囲**: ユーザー/機能/地域/時間帯、SLO/ビジネス影響
3. **仮説**: 可能性の高い原因候補（根拠付き）
4. **Cheap test（低コスト検証）**: 低コストで検証できる観察/実験（Read-only（読み取り専用）優先）
5. **恒久対策**: 再発防止の修正/運用改善（ロールバック/監視追加を含む）

必要に応じて、各項目に安全性ラベル（`[Read-only]` / `[状態変更]` / `[破壊的]`）を付けてください。
...
次に読む： [第6章：トラブルからの学びと再発防止](../chapter06/) / [目次（トップ）](../../)
