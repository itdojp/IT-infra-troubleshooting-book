...
| `du -sh path` | ファイル/ディレクトリ使用量 | 容量を消費するディレクトリ特定 |
| `lsblk` | ブロックデバイス一覧 | ストレージ構成確認 |
| `lspci` | PCIデバイス一覧 | ハードウェア構成確認 |

補足（メモリ）:
- Linux は空きメモリをキャッシュに利用するため、`free` の見た目だけで不足と判断しない方が安全です。
- `free -h` の `available` は「追加で利用可能なメモリ」の目安として扱えます。

補足（ディスク）:
- `df` はファイルシステム全体のブロック使用量、`du` はファイル/ディレクトリ配下の実体の合計（スキャン結果）を示します。
- `df` が大きいのに `du` が小さい場合は、削除済みだがプロセスが掴んでいるファイル（unlink 済み）、スナップショット/メタデータ、予約領域などを疑います。
...
| `kill -TERM PID` | プロセス終了要求 | 通常の停止（まずはこちら） |
| `kill -9 PID` | プロセス強制終了（最後の手段） | 応答なしプロセス終了（後始末が走らない可能性） |

注意: `kill -9`（SIGKILL）はプロセス側の後始末（ログ出力/一時ファイル削除/トランザクション処理など）をスキップする可能性があります。可能なら SIGTERM（`kill -TERM`）で止め、影響範囲とロールバック手段を確認したうえで段階的に対応してください。

補足（CPU）: `top` の `Cpu(s)` 行の主な項目
- `us`: ユーザー空間（アプリケーションの計算処理など）
- `sy`: カーネル空間（システムコール処理など）
- `wa`: I/O待機（ストレージI/O待ちなど）
