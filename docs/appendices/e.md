...
- **証拠**: 特定AZ（ap-northeast-1a）でのインスタンス/ネットワーク健全性低下、ヘルスチェック失敗、フェイルオーバーイベント、CloudWatch/ステータスチェックの異常
- **判断**: AZ単位の障害・劣化として扱い、影響を受けるリソース/経路を切り離して別AZへ退避するのが最短
- **行動**: 影響AZの切り離し（ELB/ASG/ルーティング）、フェイルオーバー/スケール/再配置で止血し、監視/自動化を強化
- **学び**: AZ健全性の常時監視、切り離しと退避の手順整備、定期的なフェイルオーバー訓練（GameDay/DR演習）
...
- **証拠**: 失敗ログインの急増（例: fail2ban）、不審なアクセスパターン（SQLi等）、監査ログにおける権限外操作の兆候
- **判断**: 不正アクセスの可能性を前提に「封じ込め→証拠保全→影響評価→復旧」の順で進める必要がある
- **行動**: 影響範囲の切り分け、攻撃元遮断/資格情報の無効化、ログ保全と調査、脆弱性修正と監視強化
- **学び**: 予防（WAF/安全な実装/最小権限）、検知（自動化）、対応（手順書/外部連携）の三点を平時から整備する
...
- **証拠**: レスポンス時間の段階的悪化、エラー率/クエリ時間の上昇、`EXPLAIN` でのフルスキャン等の兆候
- **判断**: ボトルネックはアプリだけでなくDB/クエリ設計にある可能性が高く、観測（メトリクス/実行計画）で仮説を絞るのが有効
- **行動**: 影響の大きい経路を特定し、実行計画の確認→インデックス/クエリ最適化→効果検証の順で恒久対応
- **学び**: 平時のベースライン把握、性能監視（SLO/SLI）と負荷試験、変更時の性能回帰チェックを仕組みに組み込む
...
- **証拠**: 設定変更直後のエラー率/レスポンス悪化、差分（設定ファイル/デプロイ）の存在、ログ上の構文エラーやリロード失敗
- **判断**: 変更起因の可能性が高いため、まず影響緩和（ロールバック）を優先しつつ、変更管理プロセスの欠陥も同時に評価する
- **行動**: 影響のある設定の即時復旧、構文/ヘルスチェック、段階的ロールアウトと監視調整、手順書/チェックリスト整備
- **学び**: 環境識別、変更管理（テスト/段階適用）、人的ミス防止（ペア作業/チェックリスト）を仕組みにする
