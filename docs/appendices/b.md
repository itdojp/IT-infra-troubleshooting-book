...
#### 2. 緊急措置
- [ ] 実行前ゲート確認（承認者、停止線、ロールバック、証跡保全、連絡先）
- [ ] 影響範囲の限定措置
- [ ] 代替手段の提供
- [ ] データ保護措置
- [ ] ログ・証跡保全

緊急措置は速度が重要ですが、状態変更を伴う操作は必ず「実行前ゲート」を通します。
特に、サービス再起動、通信遮断、権限変更、データ復旧、フェイルオーバーでは、
実行コマンド、対象、期待結果、戻し手順、実行後の確認観点を1セットで記録してください。
暫定対応であっても、誰が、いつ、なぜ実行したかを後から追跡できる状態を維持します。
